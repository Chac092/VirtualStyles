...
public class prendasEjemplo extends AppCompatActivity {
    ...
    public void Rellenar_conjuntos(String usuario){
        ...
        //comprobamos que la base de datos no existe
        String[] projectiondb = {DBHelper.entidadConjunto._ID};
        // String sortOrdergorros = DBHelper.entidadPrenda._ID + " DESC";
        Cursor cursordb = db.query(DBHelper.entidadConjunto.TABLE_NAME, projectiondb, null, null, null, null, null);
        System.out.println(cursordb.getCount());
        if (cursordb.getCount()==0){
            System.out.println("No hay conjuntos los creamos");
            ...
            for (int p1 = 0;p1<gorros.size();p1++){
                ...
                for (int p2 = 0; p2 <camisa.size();p2++){
                    prenda2 = Integer.parseInt(camisa.get(p2));
            ...
            for (int k = 0; k<Conjuntos.size();k++){
                ...
                System.out.println(Conjuntos.get(k).getIdConjunto());
                System.out.println(Conjuntos.get(k).getPrenda1());
                System.out.println(Conjuntos.get(k).getPrenda2());
                System.out.println(Conjuntos.get(k).getPrenda3());
                System.out.println(Conjuntos.get(k).getPrenda4());
                System.out.println(Conjuntos.get(k).getIdUsuario());
